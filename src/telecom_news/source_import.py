...
* колонка MIME решает вид источника: ``application/json`` → ``status``,
  ``application/rss+xml`` / ``application/xml`` / ``text/xml`` → ``news``;
...
_STATUS_TOPIC_MARKERS = ("status", "incident", "мониторинг", "доступност")
...
def classify_kind(mime: str, source_type: str, topic: str, url: str) -> str:
    """Вид источника по MIME, типу, тематике и URL."""
    haystack = f"{mime} {source_type} {topic}".lower()
    if "json" in (mime or "").lower():
        return KIND_STATUS
    lowered_url = (url or "").lower()
    if any(marker in lowered_url for marker in _STATUS_URL_MARKERS) and "feed" not in lowered_url:
        return KIND_STATUS
    if any(marker in haystack for marker in _STATUS_TOPIC_MARKERS) and "rss" not in mime.lower():
        return KIND_STATUS
    return KIND_NEWS
...
def parse_rows(
    ...
    *,
    ...
    needed = {"url", "publisher"}
    missing = [field for field in needed if field not in mapping]
    if missing:
        raise ValueError(
            f"cannot find column(s) {missing} in the table; headers seen: {list(rows[0].keys())}"
        )
...
def read_table(path: Path | str) -> list[dict[str, str]]:
    """Прочитать CSV/TSV или XLSX в список словарей (первая строка — заголовки)."""
    path = Path(path)
    if not path.exists():
...
def _read_xlsx(path: Path) -> list[dict[str, str]]:
    ...
    return [
        {headers[index]: str(cell if cell is not None else "") for index, cell in enumerate(row)}
        for row in rows[1:]
        if row
    ]
