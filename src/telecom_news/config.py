...
class Config:
    ...
    def __post_init__(self) -> None:
        ...
        if max_age:
            try:
                object.__setattr__(self, "article_max_age_days", max(0, int(float(max_age))))
            except ValueError:
                pass
...
def _apply_disabled_override() -> None:
    """Force-disable sources listed in ``TELECOM_NEWS_DISABLED_SOURCES``.

    The catalog is regenerated by ``sources import``, so a manual ``enabled=False``
    there does not survive. This env var does: it is the supported way to switch
    off a dead feed (404/403, dormant since 2021, off-topic) without touching
    generated code. Unknown ids fail loudly instead of being ignored.
    """
    raw = os.environ.get("TELECOM_NEWS_DISABLED_SOURCES", "")
    ids = {part.strip() for part in raw.split(",") if part.strip()}
    if not ids:
        return
    unknown = sorted(ids - set(SOURCES))
    if unknown:
        raise ValueError(
            "TELECOM_NEWS_DISABLED_SOURCES lists unknown source id(s): " + ", ".join(unknown)
        )
    for source_id in ids:
        SOURCES[source_id] = replace(SOURCES[source_id], enabled=False)
