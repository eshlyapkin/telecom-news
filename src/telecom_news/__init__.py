"""telecom-news: daily monitoring and publishing of SMS/messaging industry news.

CLI pipeline (M0–M8, D-020) plus multi-project foundation (M9a / D-021):
project registry, optional FastAPI GUI. The control panel covers pause/queue/
sources (M9b), run-now and AI rules (M9c), source add/delete (M9d), the
published-posts view and source discovery (M10).
See ``docs/ROADMAP.md`` and ``docs/VISION_MULTI_PROJECT.md``.
"""

__version__ = "0.7.0"
