...
| `PUBLISH_BACKLOG_PER_DAY` | 6 | сколько статей старше окна уходит в канал за сутки капельно (`0` = не публиковать их вовсе, D-031) |
| `PUBLISH_BACKLOG_MIN_GAP_HOURS` | 2 | минимальный интервал между двумя такими публикациями |

Оба параметра правятся во вкладке Overview панели («Archive pace»); сохранённое там значение перекрывает env и действует со следующего цикла без перезапуска.
